...
const Dashboard = () => {
  ...
  const { appointments, isLive } = useAppointments();
  const [selectedAppointmentId, setSelectedAppointmentId] = useState<string>('');

  const effectiveSelectedId = selectedAppointmentId || (appointments[0]?.id ?? '');
  const selectedAppointment = appointments.find(a => a.id === effectiveSelectedId) || appointments[0];
      ...
      <div className="mb-8 flex items-end justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1 flex items-center gap-2">
            ...
            {isLive ? (
              <span className="inline-flex items-center gap-1 text-xs text-green-600"><Wifi size={12} /> Live</span>
            ) : (
              <span className="inline-flex items-center gap-1 text-xs text-slate-400"><WifiOff size={12} /> Demo</span>
            )}
          </p>
        </div>
      </div>
