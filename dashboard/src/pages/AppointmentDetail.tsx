...
const APPOINTMENT_CACHE_KEY = "westpac_appointment_detail_cache_v1";
const appointmentMemoryCache = new Map<string, Appointment>();

function readAppointmentCache(): Record<string, Appointment> {
  try {
    const raw = localStorage.getItem(APPOINTMENT_CACHE_KEY);
    if (!raw) return {};
...
function writeAppointmentToCache(appointment: Appointment) {
  appointmentMemoryCache.set(appointment.id, appointment);
  const current = readAppointmentCache();
  current[appointment.id] = appointment;
  localStorage.setItem(APPOINTMENT_CACHE_KEY, JSON.stringify(current));
}

function getCachedAppointment(id?: string): Appointment | undefined {
  if (!id) return undefined;
  const mem = appointmentMemoryCache.get(id);
  if (mem) return mem;
  const disk = readAppointmentCache()[id];
  if (disk) {
    appointmentMemoryCache.set(id, disk);
    return disk;
  }
...
const AppointmentDetail = () => {
  ...
  const handleAccept = async () => {
    ...
    setAccepting(true);
    try {
      let acceptResult: Awaited<ReturnType<typeof acceptSlot>> | null = null;

      // Use preferred_slot_id if available, otherwise fetch first available slot
      const slotId =
        (appointment as any).preferredSlotId ||
        (appointment as any).preferred_slot_id;
      if (slotId) {
        acceptResult = await acceptSlot(id, slotId);
      } else {
        // Fetch slots and use the first one
        const slots = await fetchBankerSlots();
        if (slots.length > 0) {
          acceptResult = await acceptSlot(id, slots[0].id);
        } else {
          throw new Error("No available banker slots to accept");
        }
      }

      setActionDone("accepted");

      const smsStatus = acceptResult?.sms?.confirmation?.status;
      if (smsStatus === "sent") {
        setSmsStatus(
          "Confirmation SMS sent to customer. Cross-sell SMS will follow in 10 seconds.",
        );
      } else if (smsStatus === "skipped") {
        setSmsStatus(
          "Booking accepted. SMS was skipped because Twilio is not configured yet.",
        );
      } else if (smsStatus === "error") {
        setSmsStatus(
          "Booking accepted, but SMS failed to send. Check Twilio logs/credentials.",
        );
      } else {
        setSmsStatus("Booking accepted. SMS status unavailable.");
      }

      // Refresh appointment data
      const updated = await fetchAppointment(id);
      setAppointment(updated);
      writeAppointmentToCache(updated);
    } catch (e) {
      console.error("Accept failed:", e);
      setSmsStatus("Could not accept booking. Please retry.");
    } finally {
      setAccepting(false);
    }
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-7">
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-3 min-h-120">
              <div className="h-5 w-40 bg-slate-200 rounded animate-pulse" />
              <div className="h-16 w-full bg-slate-100 rounded animate-pulse" />
              <div className="space-y-2">
                <div className="h-12 w-4/5 bg-slate-100 rounded-xl animate-pulse" />
                <div className="h-12 w-3/5 ml-auto bg-slate-100 rounded-xl animate-pulse" />
                <div className="h-12 w-2/3 bg-slate-100 rounded-xl animate-pulse" />
                <div className="h-12 w-1/2 ml-auto bg-slate-100 rounded-xl animate-pulse" />
              </div>
            </div>
          </div>
        </div>
    ...
    return (
      <div className="p-8 text-center bg-white rounded-lg shadow-sm border border-slate-200">
        ...
        <p className="text-slate-500 mt-2">
          This appointment may have been removed or is not available yet.
        </p>
        <Link
          to="/"
          className="text-[#DA1710] hover:underline mt-4 inline-block"
        >
          Return to Dashboard
        </Link>
      </div>
  ...
  const getSentimentColor = (sentiment: string) => {
    switch (sentiment) {
      ...
      default:
        return { text: "text-slate-600", bg: "bg-slate-400" };
    }
  };
        ...
        <div className="flex gap-3 items-center">
            ...
            <>
              ...
              >
                {accepting ? (
                  ...
                  <>
                    Accept Booking <CheckCircle2 size={16} />
                  </>
                )}
              </button>
            </>
          ...
          {(appointment.status === "Upcoming" || actionDone === "accepted") && (
            <>
              ...
              {(appointment.locationType === "Video chat" ||
              ...
              <div className="flex items-center gap-4">
                ...
                {appointment.estimatedLoanSize && (
                  <div className="text-right">
                    <div className="text-2xl font-bold text-[#DA1710]">
                      {appointment.estimatedLoanSize}
                    </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            ...
            {appointment.reasonForLeaving && (
              <div className="text-sm text-slate-500 mt-1">
                Reason: {appointment.reasonForLeaving}
              </div>
            )}
          </div>
            ...
            <div className="bg-slate-100 border border-slate-200 p-4 rounded-xl flex gap-3 shadow-sm">
              ...
              <div>
                <h4 className="font-bold text-slate-800 text-sm">
                  {appointment.recommendedStrategy.title}
                </h4>
            ...
            <div className="px-5 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {appointment.transcript.length} messages
                </p>
              </div>
              {/* Sentiment Badge */}
              <div className="flex items-center gap-2">
                <span className={`text-xs font-medium ${sentimentColors.text}`}>
                  {appointment.sentiment}
                </span>
                <div className="w-16 bg-slate-200 h-1.5 rounded-full overflow-hidden">
                  <div
                    className={`h-full ${sentimentColors.bg}`}
                    style={{ width: `${appointment.sentimentScore}%` }}
                  ></div>
                </div>
              </div>
            </div>
