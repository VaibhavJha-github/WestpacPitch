...
async def live_session(ws: WebSocket):
    ...
    async def do_tts_stream(text, lang):
        """Background TTS streaming — cancellable on interruption."""
        try:
            async for chunk in stream_speech(text, language_code=lang):
    ...
    try:
        # AI greets first
        customer_name = profile["full_name"].split()[0] if profile else "there"
        greeting = f"Hi {customer_name}, thanks for calling Westpac. How can I help you today?"
        messages.append({"role": "assistant", "content": greeting})
        all_turns.append({"speaker": "bot", "text": greeting, "lang": "en"})

        await ws.send_json({"type": "response_text", "text": greeting})
        tts_task = asyncio.create_task(do_tts_stream(greeting, "en"))
        ...
        while True:
            ...
            if msg_type == "audio":
                ...
                # Stream TTS in background — can be cancelled on interruption
                tts_task = asyncio.create_task(do_tts_stream(bot_text, voice_lang))
